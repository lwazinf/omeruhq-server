...
const tierPrices = [199, 499, 999];
const tierCommissions = ['5%', '3.5%', '2.5%'];
const tierFeaturedIndex = 1;
const tierFeatureCounts = [6, 6, 7];
...
export default function Pricing() {
  ...
  const tiers = [0, 1, 2].map((i) => ({
    ...
    featured: i === tierFeaturedIndex,
    features: Array.from({ length: tierFeatureCounts[i] }, (_, j) => t(`tiers.${i}.features.${j}`)),
    cta: t(`tiers.${i}.cta`),
    note: t(`tiers.${i}.note`),
  }));
          ...
          {tiers.map((tier, i) => (
            ...
            >
              ...
              >
                ...
                {tier.featured && (
                  <div style={{ position: 'absolute', top: -11, left: '50%', transform: 'translateX(-50%)', zIndex: 2, background: 'var(--lime)', color: 'var(--black)', borderRadius: 100, padding: '3px 14px', fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    {t('mostPopular')}
                  </div>
                )}
                ...
                <div style={tier.featured ? { position: 'relative', zIndex: 1 } : {}}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 2, marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 13, color: tier.featured ? 'rgba(255,255,255,0.35)' : 'var(--mid-gray)', paddingTop: 10, lineHeight: 1 }}>{t('perMonth')}</span>
                  </div>

                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: tier.featured ? 'rgba(200,241,53,0.12)' : 'var(--warm-gray)', borderRadius: 100, padding: '3px 10px', fontSize: 11, fontWeight: 600, color: tier.featured ? 'var(--lime)' : 'var(--black)', marginBottom: 16 }}>
                    + {tier.commission} {t('perSale')}
                  </div>
                  ...
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 11, marginBottom: 32 }}>
                    {tier.features.map((f, j) => (
                      <li key={j} style={{ display: 'flex', alignItems: 'flex-start', gap: 9, fontSize: 13, color: tier.featured ? 'rgba(255,255,255,0.75)' : 'var(--dark-gray)', lineHeight: 1.5 }}>
                  ...
                  <button
                    ...
                    style={{ display: 'block', textAlign: 'center', width: '100%', padding: '13px 20px', borderRadius: 100, background: tier.featured ? 'var(--lime)' : 'transparent', color: 'var(--black)', border: tier.featured ? 'none' : '1.5px solid rgba(0,0,0,0.15)', fontWeight: 500, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s ease', marginBottom: 10 }}
