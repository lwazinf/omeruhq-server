  ...
  const columns = [
    ...
    },
    {
      ...
      links: [
        ...
        { label: 'sales@omeru.io',      href: 'mailto:sales@omeru.io'      },
  ...
  return (
    <footer ref={ref} style={{ background: 'var(--black)', padding: 'clamp(60px, 8vw, 80px) 0 36px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} style={{ marginBottom: 'clamp(48px, 7vw, 72px)', textAlign: 'center' }}>
          <h2 className="display-lg" style={{ color: 'white', marginBottom: 20 }}>
            {t('ctaHeading')}<br />
            <span style={{ color: 'var(--lime)' }}>{t('ctaHeadingAccent')}</span>
          </h2>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', fontWeight: 300 }}>{t('copyright')}</span>
          <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', fontWeight: 300 }}>{t('madeIn')}</span>
          </div>
        </div>
      </div>
