...
> **This release**
> - 🗓️ **Services & bookings** — merchants publish bookable services; customers pick a day and a free slot generated from store hours; merchants accept/decline/manage from a 7-day diary; hourly reminders.
> - 🖼️ **Durable images** — merchant photos are persisted to Supabase Storage at upload time. The old behaviour stored WhatsApp media IDs, which Meta expires (~30 days) — that bug is fixed.
> - 🌐 **Web storefronts** — going live now also creates `omeru.io/@handle`, an SEO-indexed page reading the same database. Every CTA deep-links back into this bot.
> - 💳 **Stitch only** — PayFast and Ozow code removed entirely.
> - 🖥️ **Terminal simulator** — `npm run cli` drives the real bot in your terminal (see below).
> - 📊 **Richer stats** and 📣 **platform announcements** for admin marketing.
...
- Pay via Stitch instant-EFT payment link sent in chat
...
- **My Bookings** — view, track and cancel service appointments
- Wishlist
- Order history
...
Test the entire bot from your terminal — same handlers, same database, zero WhatsApp credentials needed. What renders in the terminal is exactly what renders on a phone.

```bash
npm run cli                      # chat as the default test number
npm run cli -- 27741234567      # chat as any wa_id
```
...
| `1`, `2`, `3` … | taps the numbered button / list row from the last message |
| `/img` | simulates sending a photo |
| `/loc -33.92,18.42` | simulates sharing a location |
| `/as 27749999999` | switch which number you are chatting as (e.g. flip between a customer and the merchant to accept your own booking request) |
| `/exit` | quit |

Only `DATABASE_URL` is required. Note: `/img` produces a fake media ID, so image *persistence* (Supabase upload) is exercised only against the live WhatsApp API.

---

## Services & Bookings

Service businesses (hair, nails, tutoring, repairs) manage everything under **💼 Services** on the merchant dashboard:

- **Add a service** — name → price → duration → photo. Live in under a minute.
- **Requests inbox** — accept or decline (with a reason) incoming booking requests.
- **7-day diary** — every confirmed appointment, grouped by day; mark done or cancel.

Customers see **📅 Book a Service** on any store that has services, pick a day from the next 7, then a free slot. Slots are generated from the merchant's opening hours minus existing bookings, with a write-time clash guard. Both sides get an automatic reminder within 24 h of the appointment.
