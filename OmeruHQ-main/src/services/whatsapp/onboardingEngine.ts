...
const handleImage = async (from: string, input: string, merchant: Merchant, message?: any): Promise<void> => {
    ...
    const persistedUrl = imageId ? await persistWhatsAppImage(imageId, `merchants/${merchant.id}`) : null;
    await db.merchant.update({ where: { id: merchant.id }, data: { image_url: persistedUrl } });
...
const handleGolive = async (from: string, input: string, merchant: Merchant): Promise<void> => {
    if (input === 'ob_golive_launch') {
        ...
        const storefrontBase = process.env.STOREFRONT_BASE_URL || 'https://omeru.io';
        await sendButtons(from,
            `🚀 *${activated.trading_name} is now LIVE!*\n\n` +
            `Customers can find you at *@${activated.handle}*.\n\n` +
            `🌐 Your web storefront is live too:\n${storefrontBase}/@${activated.handle}\n` +
            `Share it anywhere — it's indexed by Google and every tap opens your WhatsApp store.\n\n` +
            `Welcome to Omeru! 🎉`,
            [
