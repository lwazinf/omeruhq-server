...
const STATE = {
    ...
    ANNOUNCE_MSG: 'PA_ANNOUNCE_MSG',
    ANNOUNCE_CONFIRM: 'PA_ANNOUNCE_CONFIRM'
};
...
export const handlePlatformAdminActions = async (
    ...
    // ── Platform announcements (marketing) ──────────────────────────────────
    // Broadcast a message to all merchants, all customers, or everyone.
    if (input === 'pa_announce') {
        const [merchants, customers] = await Promise.all([
            db.merchant.count({ where: { status: MerchantStatus.ACTIVE } }),
            db.userSession.count({ where: { mode: 'CUSTOMER' } })
        ]);
        await sendButtons(from,
            `📣 *Platform Announcement*\n\nWho should receive it?\n\n🏪 Active merchants: ${merchants}\n🛍️ Customers: ${customers}`,
            [
                { id: 'pa_ann_merchants', title: '🏪 Merchants' },
                { id: 'pa_ann_customers', title: '🛍️ Customers' },
                { id: 'pa_ann_all', title: '🌍 Everyone' }
            ]);
        return;
    }

    if (['pa_ann_merchants', 'pa_ann_customers', 'pa_ann_all'].includes(input)) {
        const audience = input.replace('pa_ann_', '');
        await setState(from, STATE.ANNOUNCE_MSG, { audience });
        await sendTextMessage(from,
            `✍️ *Type your announcement.*\n\nIt will be sent to *${audience}* exactly as written — emojis and *bold* work.\n\n_Type "Omeru" to cancel._`);
        return;
    }

    if (state === STATE.ANNOUNCE_MSG) {
        const audience = payload?.audience || 'all';
        await setState(from, STATE.ANNOUNCE_CONFIRM, { audience, message: input.substring(0, 1500) });
        await sendButtons(from,
            `📋 *Preview — to ${audience}:*\n\n${input.substring(0, 1500)}`,
            [
                { id: 'pa_ann_send', title: '🚀 Send Now' },
                { id: 'pa_menu', title: '❌ Cancel' }
            ]);
        return;
    }

    if (input === 'pa_ann_send' && state === STATE.ANNOUNCE_CONFIRM) {
        const audience = payload?.audience || 'all';
        const messageText = payload?.message || '';
        await clearState(from);
        if (!messageText) { await sendTextMessage(from, '⚠️ Session expired. Start again.'); return; }

        const recipients = new Set<string>();
        if (audience === 'merchants' || audience === 'all') {
            const merchants = await db.merchant.findMany({
                where: { status: MerchantStatus.ACTIVE }, select: { wa_id: true }
            });
            merchants.forEach(m => recipients.add(m.wa_id));
        }
        if (audience === 'customers' || audience === 'all') {
            const sessions = await db.userSession.findMany({
                where: { mode: 'CUSTOMER' }, select: { wa_id: true }
            });
            sessions.forEach(s => recipients.add(s.wa_id));
        }
        recipients.delete(from);

        await sendTextMessage(from, `🚀 Sending to ${recipients.size} recipients…`);
        let sent = 0;
        for (const waId of recipients) {
            const ok = await sendTextMessage(waId, `📣 *Omeru*\n\n${messageText}`);
            if (ok) sent++;
        }
        await log(AuditAction.ADMIN_ANNOUNCEMENT, from, 'Platform', 'announcement', { audience, sent, message: messageText.substring(0, 200) });
        await sendButtons(from, `✅ Announcement delivered to ${sent}/${recipients.size}.`, [
