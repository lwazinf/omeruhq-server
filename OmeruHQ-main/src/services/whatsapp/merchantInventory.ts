...
import { persistWhatsAppImage, deletePersistedImage } from '../media/storage';
    ...
    try {
        ...
        if (stateKey === STATE.EDIT_PROD_IMG) {
            ...
            if (message?.type === 'image' && message?.image?.id) {
                const old = await db.product.findUnique({ where: { id: pid }, select: { image_url: true } });
                const imageUrl = await persistWhatsAppImage(message.image.id, `products/${pid}`);
                await db.product.update({ where: { id: pid }, data: { image_url: imageUrl } });
                await deletePersistedImage(old?.image_url);
                await logAudit({ actorWaId: from, action: 'PRODUCT_UPDATED', entityType: 'PRODUCT', entityId: pid, metadata: { image_url: imageUrl } });
                await clearState(from);
        ...
        if (stateKey === STATE.IMAGE) {
            ...
            if (message?.type === 'image' && message?.image?.id) {
                imageUrl = await persistWhatsAppImage(message.image.id, `products/${pid}`);
