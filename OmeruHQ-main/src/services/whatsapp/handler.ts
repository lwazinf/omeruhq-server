...
export const handleIncomingMessage = async (message: any): Promise<void> => {
    ...
    try {
        ...
        // Easter egg
        if (normalizedInput === 'hilwazi') {
            await sendTextMessage(from, 'Hi Sir');
            return;
        }
        ...
        if (input === 'sw_admin') {
            if (!isPlatformAdmin(from)) {
                await sendTextMessage(from, '⛔ Access denied.');
                return;
            }
            await db.userSession.update({ where: { wa_id: from }, data: { mode: 'CUSTOMER', active_merchant_id: null, active_prod_id: null, state: null } });
            await handlePlatformAdminActions(from, 'admin');
        ...
        if (input.startsWith('sw_merchant_')) {
            ...
            const updatedSession = await db.userSession.update({
                where: { wa_id: from },
                data: { mode: 'MERCHANT', active_merchant_id: merchantId, active_prod_id: null, state: null }
            });
        ...
        // Customer bookings — services calendar flow
        if (input === 'c_my_bookings' || input.startsWith('c_book_') || input.startsWith('cbk_')) {
            await handleCustomerBookings(from, input);
            return;
        }
        ...
        // Retry payment — resend Stitch payment link for an existing order
...
const processInviteAccept = async (waId: string, invite: any, accept: boolean): Promise<void> => {
    if (accept) {
        ...
        await db.userSession.upsert({
            where: { wa_id: waId },
            update: { mode: 'MERCHANT', active_merchant_id: invite.merchant_id, active_prod_id: null, state: null },
