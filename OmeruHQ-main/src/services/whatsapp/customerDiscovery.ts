...
const processBuyNow = async (
    ...
    try {
        const { paymentUrl, transactionRef } = await createPaymentRequest({
            orderId:      order.id,
            amount:       price,
            merchantName: merchant.handle.substring(0, 20),
            customerWaId: from,
        });
...
const sendStoreProductPage = async (from: string, merchantHandle: string, page: number, platformBranding: any, sortCode?: string): Promise<void> => {
    ...
    if (products.length === 0 && page === 1) {
        const serviceCount = await db.service.count({ where: { merchant_id: merchant.id, is_active: true } });
        if (serviceCount > 0) {
            await sendButtons(from,
                `💼 *${merchant.trading_name}* is a service business — pick a service to book a time!`,
                [{ id: `c_book_${merchant.id}`, title: '📅 Book a Service' }, { id: 'browse_shops', title: '🛍️ Browse' }]
            );
            return;
        }
        await sendButtons(from,
    ...
    // Bookable services bubble — page 1 only
    if (page === 1) {
        const serviceCount = await db.service.count({ where: { merchant_id: merchant.id, is_active: true } });
        if (serviceCount > 0) {
            await sendButtons(from,
                `💇 *${merchant.trading_name}* also takes bookings!`,
                [{ id: `c_book_${merchant.id}`, title: '📅 Book a Service' }]);
        }
    }
