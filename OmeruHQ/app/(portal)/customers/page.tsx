...
export default async function CustomersPage() {
  ...
  const [customers, totalCount, optedInCount, bookmarkedCount] = await Promise.all([
    ...
    <div className="portal-page" style={{ padding: 'clamp(20px, 3vw, 36px) clamp(16px, 3vw, 36px)', maxWidth: 1000 }}>
      ...
      {customers.length === 0 ? (
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {/* Table header */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 100px 96px', gap: 0, padding: '10px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.03)' }}>
            {['Customer', 'Last seen', 'Broadcast', 'Actions'].map(h => (
          ...
          {customers.map((c, i) => (
            <div
              key={c.id}
              style={{
                display: 'grid', gridTemplateColumns: '1fr 110px 100px 96px', gap: 0,
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                <div style={{ width: 34, height: 34, borderRadius: '50%', flexShrink: 0, background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.4)' }}>
                  {(c.display_name ?? c.wa_id).charAt(0).toUpperCase()}
                </div>
                <div style={{ minWidth: 0 }}>
                  ...
                  {c.display_name && (
                    <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 1 }}>{c.wa_id}</div>
                  )}
                </div>
              </div>
              ...
              <div>
                <span style={{
                  ...
                  background: c.opt_out ? 'rgba(255,255,255,0.05)' : 'rgba(200,241,53,0.1)',
                  color: c.opt_out ? 'var(--mid-gray)' : 'var(--lime)',
                  border: `1px solid ${c.opt_out ? 'rgba(255,255,255,0.08)' : 'rgba(200,241,53,0.2)'}`,
                }}>
