    ...
    try {
        ...
        if (state === STATE.LOGO) {
            if (message?.type === 'image' && message?.image?.id) {
                const storeImageUrl = await persistWhatsAppImage(message.image.id, `merchants/${merchant.id}`);
                await db.merchant.update({ where: { id: merchant.id }, data: { image_url: storeImageUrl } });
        ...
        if (state === STATE.WELCOME_IMG) {
            if (message?.type === 'image' && message?.image?.id) {
                const welcomeImageUrl = await persistWhatsAppImage(message.image.id, `merchants/${merchant.id}/welcome`);
                await db.merchant.update({ where: { id: merchant.id }, data: { welcome_image_url: welcomeImageUrl } as any });
                await clearState(from);
